...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM User u JOIN u.userPreferenceReference up WHERE up.okToMail = true")
    List<User> findUsersWithMailPreference();
...
//    List<User> findUsersByMailPreferenceNative(@Param("okToMail") boolean okToMail);
    ...
    @Query(value = "SELECT u.* FROM user_table u JOIN user_preference up ON u.id = up.user_id WHERE up.ok_to_mail = true", nativeQuery = true)
    List<User> findUsersWithMailPreferenceNative();
}
